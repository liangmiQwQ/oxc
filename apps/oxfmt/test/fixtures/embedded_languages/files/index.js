...
const styles = css`
  .button {color: red;background: blue;padding: 10px 20px;}
  .container {display: flex;justify-content: center;}
`;

const styledComponent = styled`
  background-color: #ffffff;border-radius: 4px;padding: 8px;
`;
...
const template = html`
  <div class="container"><h1>Hello World</h1><p>This is a paragraph with <strong>bold</strong> text.</p></div>
`;
...
// prettier-ignore
const unformatted = css`
  .button {color: red;background: blue;border: 1px solid green;}
`;
...
// prettier-ignore
const ignoredGql = gql`
  query GetUser($id: ID!) {user(id: $id) {name email}}
`;

const normalGql = gql`
  query GetPosts {posts {title author}}
`;
