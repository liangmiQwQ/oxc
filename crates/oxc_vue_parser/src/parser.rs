...
impl<'a> ErrorHandler for OxcErrorHandler<'a> {
    fn on_error(&self, error: vue_compiler_core::error::CompilationError) {
        self.errors.borrow_mut().push(OxcDiagnostic::error(error.to_string()))
    }
}
...
impl<'a> VueOxcParser<'a> {
    ...
    pub fn parse(mut self) -> ParserReturn<'a> {
                    ...
                    if node.tag_name == "script" {
                        ...
                        children.push(self.parse_element(
                            ...
                            Some(ast.vec1(ast.jsx_child_expression_container(
                                SPAN,
                                JSXExpression::ArrowFunctionExpression(
        ...
        ParserReturn { program, errors: errors.take() }
    }
    ...
    fn parse_element(
        ...
        ast.jsx_child_element(
            ...
            ast.jsx_opening_element(
                ...
                NONE,
                ast.vec_from_iter(
                    node.properties.into_iter().map(|prop| self.parse_attribute(prop)),
                ),
            ),
                    ...
                    match dir.name {
                                ...
                                if let DirectiveArg::Dynamic(_) = argument {
                                    ...
                                    if head_name.starts_with(":") {
                                        ...
                                        ast.jsx_attribute_name_identifier(
                                            Span::new(
                                                dir_start + 1,
                                                dir.head_loc.end.offset as u32,
                                            ),
                                            self.parse_argument(
                                                dir.argument.as_ref().unwrap(),
                                                modifiers,
                                            ),
                                        )
                                    } else {
                                        ...
                                        ast.jsx_attribute_name_namespaced_name(
                                            dir.head_loc.span(),
                                            ast.jsx_identifier(
                                                Span::new(dir_start, dir_start + 6),
                                                ast.atom("v-bind"),
                                            ),
                                            ast.jsx_identifier(
                                                Span::new(
                        ...
                        _ => {
                            if let Some(argument) = &dir.argument {
                                ...
                                match argument {
                                    DirectiveArg::Static(arg) => ast
                                        .jsx_attribute_name_namespaced_name(
                                            dir.head_loc.span(),
                                            ast.jsx_identifier(
                                            ...
                                            ast.jsx_identifier(
                                                if head_name.starts_with("v-") {
                                                    Span::new(
                                                        namespace_end + 1,
                                                        namespace_end + 1 + arg.len() as u32,
                                                    )
                                                } else {
                                                    Span::new(
                                                        namespace_end,
                                                        namespace_end + arg.len() as u32,
                                                    )
                                                },
                                                self.parse_argument(
                                                    dir.argument.as_ref().unwrap(),
    ...
    fn parse_modifiers(&self, modifiers: Vec<&'a str>) -> String {
        if !modifiers.is_empty() { format!("_{}", modifiers.join("_")) } else { String::new() }
    }
    ...
    pub fn parse_expression(&self, source: &'a str, start: usize) -> Expression<'a> {
        ...
        let source_text = ast
            ...
            .as_str();
        let mut program =
            oxc_parser::Parser::new(self.allocator, source_text, self.source_type).parse().program;
